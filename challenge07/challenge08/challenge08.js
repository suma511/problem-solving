...
const getEntries = (obj) => {
  const pizza =new.getRecipekey(`pizza`,[`bread`,`tomato`,`chicken`,`mayo`],`two hours`,25$)
};
...
const getInfo = (arr) => {
  let coursesName = [];
  let studentsName = [];
  let coursesName = new Array('Java','Van','Python','DotNet');

for(let counter=0;counter<=length;counter++) 
  console.log(coursesName[counter]);
  console.log(studentsName[counter]);                  
...
const getStudents = (arr) => {
  let getStudent =`${this.Student} `${this.coursesName}
